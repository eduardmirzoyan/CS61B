...
public class Main {
    ...
    static final File CAPERS_FOLDER = new File(CWD.getPath() + "capers");
    ...
    public static void writeStory(String[] args) {
        ...
        String s = "";
        if (storyFile.exists()) {
            s = Utils.readContentsAsString(storyFile) + args[1] + "\n";
        }
        else {
            s = args[1] + "\n";
        }
        Utils.writeContents(storyFile, s);
    ...
    public static void makeDog(String[] args) {
        ...
        Dog newDoge = new Dog(args[1], args[2], Integer.parseInt(args[3]));
        newDoge.saveDog();
        System.out.println(newDoge.toString());
    }
