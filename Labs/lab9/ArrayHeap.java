...
public class ArrayHeap<T> {
    ...
    private void bubbleUp(int index) {
        while (index > 1 && minPriorityIndex(index, getParentOf(index)) == index) {
    ...
    private void bubbleDown(int index) {
        ...
        int minOfChildren = minPriorityIndex(getLeftOf(index), getRightOf(index));
        int minIndex = minPriorityIndex(index, minOfChildren);
        if (index != minIndex) {
            swap(index, minIndex);
            bubbleDown(minIndex);
        }
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i < size() + 1; i++) {
            if (getNode(i).item().equals(item)) {
                ...
                double previousPriority = getNode(i).priority();
