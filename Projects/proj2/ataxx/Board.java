...
class Board {
    ...
    boolean canMove(PieceColor who) {
        int doubleExtended = EXTENDED_SIDE * 2;
        for (int i = 0; i < _board.length; i++) {
            if (_board[i] == who) {
                for (int j = i - 2; j <= i + 2; j++) {
                    for (int k = j - doubleExtended;
                         k <= j + doubleExtended; k += 11) {
                        if (_board[k] == EMPTY) {
                            return true;
                        }
                    }
    ...
    void makeMove(Move move) {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                if (i != 0 || j != 0) {
                    int aNeighbor = neighbor(move.toIndex(), i, j);
                    if (get(aNeighbor) == _whoseMove.opposite()) {
                        set(aNeighbor, _whoseMove);
                        incrPieces(_whoseMove, 1);
    ...
    void undo() {
        ...
        if (!lastMove.isPass()) {
            while (_undoSquares.peek() != null && _undoPieces.peek() != null) {
                if (_undoPieces.peek() != EMPTY) {
                    incrPieces(_undoPieces.peek(), 1);
                    incrPieces(_undoPieces.peek().opposite(), -1);
                } else {
                    incrPieces(_whoseMove.opposite(), -1);
                    incrPieces(_whoseMove, 1);
                }
    ...
    boolean legalBlock(char c, char r) {
        return _allMoves.size() == 0 && get(c, r) == EMPTY;
    }
    ...
    void setBlock(char c, char r) {
        ...
        if (get(c, r) != BLOCKED) {
            ...
            if (!legalBlock(cM, r) || !legalBlock(cM, rM)
                    || !legalBlock(c, rM)  || !legalBlock(c, r)) {
                throw error("illegal block placement");
            } else {
                if (get(c, r) != BLOCKED) {
                ...
                if (get(cM, r) != BLOCKED) {
                    unrecordedSet(cM, r, BLOCKED);
                    _totalOpen--;
                }
                if (get(c, rM) != BLOCKED) {
                    unrecordedSet(c, rM, BLOCKED);
                    _totalOpen--;
                }
                if (get(cM, rM) != BLOCKED) {
                    unrecordedSet(cM, rM, BLOCKED);
                    _totalOpen--;
                }
            }
        }
